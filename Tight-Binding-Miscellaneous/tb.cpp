...
struct dot{double p1[4] = {0}; };
struct phi{comp factors[9] = {}; };
...
proj_bandvecs projection_gauss_10_pt_quad(bandvecs_and_vals bandvectors_and_values(eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, bool get_vecs),
eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),

	double sh_real[8][1331], double sh_imag[8][1331],
	double params[6], double neighbors[4][3], double akx, double bkx, double aky, double bky, double akz, double bkz){
	...
	{					
	#pragma omp for collapse(3), schedule(static), reduction(+: real_part[:8][:8], imag_part[:8][:8])
		for(int l = 0; l < 11; l++){
			for(int m = 0; m < 11; m++){
				for(int n = 0; n < 11; n++){
					double end = half_diff_kx*half_diff_ky*half_diff_kz*weights[l]*weights[m]*weights[n];
					bandvecs_and_vals TB = bandvectors_and_values(band_eigenvalues_and_eigenvectors, phase, params, neighbors,  half_diff_kx*points[l]+half_sum_kx, half_diff_ky*points[m]+half_sum_ky, half_diff_kz*points[n]+half_sum_kz,true);		
					for(int s = 0; s < N; s++){
...
phi phase(double kx, double ky, double kz, double neighbors[4][3]){
	phi p = {};
	p.factors[0].real = cos(0.5*M_PI*kx)*cos(0.5*M_PI*ky)*cos(0.5*M_PI*kz);
	p.factors[0].imag = -1*sin(0.5*M_PI*kx)*sin(0.5*M_PI*ky)*sin(0.5*M_PI*kz);
	p.factors[1].real = -1*cos(0.5*M_PI*kx)*sin(0.5*M_PI*ky)*sin(0.5*M_PI*kz);
	p.factors[1].imag = sin(0.5*M_PI*kx)*cos(0.5*M_PI*ky)*cos(0.5*M_PI*kz);
	p.factors[2].real = -1*sin(0.5*M_PI*kx)*cos(0.5*M_PI*ky)*sin(0.5*M_PI*kz);
	p.factors[2].imag = cos(0.5*M_PI*kx)*sin(0.5*M_PI*ky)*cos(0.5*M_PI*kz);
	p.factors[3].real = -1*sin(0.5*M_PI*kx)*sin(0.5*M_PI*ky)*cos(0.5*M_PI*kz);
	p.factors[3].imag = cos(0.5*M_PI*kx)*cos(0.5*M_PI*ky)*sin(0.5*M_PI*kz);
...
eigen Householder_with_QR(comp hamiltonian[N][N], bool Vecs){
	...
	for(k = 0; k<N-2;k++){
		...
		if(gamma < 0.000000000001){
			...
			alpha = 0;
			if(norm(hamiltonian[k+1][k]) < 0.00000000001){
				beta.real = 1;
				beta.imag = 0;
			}
			else{
...
eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs){
	...
	comp hamiltonian[N][N] = {{es_comp,	           mult_comp(g0,vss),	 zero_comp,		        zero_comp,			   zero_comp,			  mult_comp(g1,vsp), mult_comp(g2,vsp), mult_comp(g3, vsp) },
                              {mult_comp(g0c,vss), es_comp,			     mult_comp(g1c,-1*vsp), mult_comp(g2c,-1*vsp), mult_comp(g3c,-1*vsp), zero_comp,		 zero_comp,		    zero_comp           },
        		           	  {zero_comp,	       mult_comp(g1,-1*vsp), ep_comp,				zero_comp,			   zero_comp,			  mult_comp(g0,vxx), mult_comp(g3,vxy), mult_comp(g1, vxy)  },
        			          {zero_comp,		   mult_comp(g2,-1*vsp), zero_comp,	            ep_comp,			   zero_comp,			  mult_comp(g3,vxy), mult_comp(g0,vxx), mult_comp(g1, vxy)  },
        			          {zero_comp,		   mult_comp(g3,-1*vsp), zero_comp,	            zero_comp,	           ep_comp,			      mult_comp(g1,vxy), mult_comp(g2,vxy), mult_comp(g0, vxx)  },
        			          {mult_comp(g1c,vsp), zero_comp,		     mult_comp(g0c,vxx),	mult_comp(g3c,vxy),	   mult_comp(g1c,vxy),	  ep_comp,		     zero_comp,		    zero_comp           },
        			          {mult_comp(g2c,vsp), zero_comp,			 mult_comp(g3c,vxy),	mult_comp(g0c,vxx),	   mult_comp(g2c,vxy),	  zero_comp,		 ep_comp,			zero_comp           },
        			          {mult_comp(g3c,vsp), zero_comp,			 mult_comp(g1c,vxy),	mult_comp(g1c,vxy),	   mult_comp(g0c,vxx),	  zero_comp,		 zero_comp,		    ep_comp             }
...
bandvecs_and_vals bandvectors_and_values(eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, bool get_vecs){
	...
	phi g = phase(kx, ky, kz, neighbors);
	eigen e = band_eigenvalues_and_eigenvectors(g.factors[0], g.factors[1],  g.factors[2],  g.factors[3], params[0], params[1], params[2], params[3], params[4], params[5], get_vecs);

	for( i = 0; i < N; i++){
		b.bandvalues[i] = e.evalues[i];
		// cout << b.bandvalues[i] << endl;
		for(j = 0; j < N; j++){
...
double partial_residual(bandvecs_and_vals bandvectors_and_values(eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, bool get_vecs),
	eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, int point, double dft_bands[P][24], double dft_band_eigenvectors[P][24][N], proj_bandvecs &TB_Proj){
	
	bandvecs_and_vals BAND_STRUCTURE = bandvectors_and_values(band_eigenvalues_and_eigenvectors, phase, params, neighbors,  kx, ky, kz, false);
...
double residual(double partial_residual(bandvecs_and_vals bandvectors_and_values(eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, bool get_vecs),
	eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
...
eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	...
	double sh_real[8][1331], double sh_imag[8][1331],
	double params[6], double neighbors[4][3], double akx, double bkx, double aky, double bky, double akz, double bkz), 
	double sh_real[N][1331], double sh_imag[N][1331], double params[6], double neighbors[4][3], double fit_points[5][3], int points[5], double dft_bands[P][24], double dft_band_eigenvectors[P][24][N]){
	
	double res = 0;
	int i = 0;

	proj_bandvecs TB_Proj = projection_gauss_10_pt_quad(bandvectors_and_values, band_eigenvalues_and_eigenvectors, phase, sh_real, sh_imag,
	params, neighbors, -M_PI/a, M_PI/a, -M_PI/a, M_PI/a, -M_PI/a, M_PI/a);

	for(int i = 0; i < 5; i++){
		res += partial_residual(bandvectors_and_values, band_eigenvalues_and_eigenvectors, phase, params, neighbors,  fit_points[i][0], fit_points[i][1], fit_points[i][2], points[i] ,dft_bands, dft_band_eigenvectors, TB_Proj);
	}	
	return res;
}

void nelmin (double residual(double partial_residual(bandvecs_and_vals bandvectors_and_values(eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
	double params[6], double neighbors[4][3], double kx, double ky, double kz, bool get_vecs),
	eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	phi phase(double kx, double ky, double kz, double neighbors[4][3]),
...
eigen band_eigenvalues_and_eigenvectors(comp g0,   comp g1,   comp g2, comp g3, double es, double ep, double vss, double vsp, double vxx, double vxy, bool get_vecs),
	...
	int n, double start[10], double neighbors[4][3], 
...
//
  for ( ; ; )
  {
    ...
    y[n] = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, start, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
    *icount = *icount + 1;
    ...
    for ( j = 0; j < n; j++ )
    {
      ...
      y[j] = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, start, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
      *icount = *icount + 1;
      start[j] = x;
    }
    // cout << "here" << endl;
...
//
      ...
      ystar = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, pstar, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
      *icount = *icount + 1;
//
...
//
      if ( ystar < ylo )
      {
        ...
        y2star = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, p2star, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
        *icount = *icount + 1;
//
//  Check extension.
//
...
//
          if ( y[ihi] < y2star )
          {
            for ( j = 0; j < nn; j++ )
            {
              ...
              y[j] = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, xmin, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
...
//
    ...
    *ifault = 0;
    ...
    for ( i = 0; i < n; i++ )
    {
      del = step[i] * eps;
      xmin[i] = xmin[i] + del;
      z = residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, xmin, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
      *icount = *icount + 1;
      if ( z < *ynewlo )
      {
        *ifault = 2;
...
int main(){
	...
	double s = 0, py = 0, pz = 0, px = 0, dxy = 0, dyz = 0, dz2 = 0, dxz = 0;
	...
	while(PROCAR){	
		...
		if((((counter-4)%265)%(8+3)==0) && (counter-4)%265!=0){
			s = 0, py = 0, pz = 0, px = 0, dxy = 0, dyz = 0, dz2 = 0, dxz = 0;
			PROCAR >> throwaway >> s >> py >> pz >> px >> dxy >> dyz >> dz2 >> dxz >> throwaway >>throwaway;
	...
	double params[6] = {-4.2, 3.0,-8.13, 5.88, 1.71 , 7.51};
	...
	// double params[6] = {1.21926,	2.64689,	10.9146,	7.39687,	5.84823,	-13.0186,	-11.0428,4.84252,	-1.48558,	8.72767};
	...
	// cout << residual(partial_residual, band_eigenvalues_and_eigenvectors, phase, projection_gauss_10_pt_quad, sh_real, sh_imag, params, neighbors, fit_points, points, dft_bands, dft_band_eigenvectors);
	// double res = 0;
	...
	// 	new_params, neighbors, -M_PI/a, M_PI/a, -M_PI/a, M_PI/a, -M_PI/a, M_PI/a);

	// 	for(int i = 0; i < 5; i++){
	// 		res += partial_residual(new_params, neighbors,  fit_points[i][0], fit_points[i][1], fit_points[i][2], points[i] ,dft_bands, dft_band_eigenvectors, TB_Proj);
	// 	}	
 ...
 // 	reqmin = 10;
	...
	// }
	// ynewlo = residual(partial_residual,  projection_gauss_10_pt_quad, sh_real, sh_imag, params, neighbors,  fit_points, points, dft_bands, dft_band_eigenvectors);
	// cout << "\n";
	// cout << "  F(X) = " << ynewlo << "\n";



	// nelmin ( residual, sh_real, sh_imag, fit_points, points, n, start, neighbors,  dft_path, dft_bands, dft_band_eigenvectors, xmin, &ynewlo, reqmin, step, konvge, kcount, &icount, &numres, &ifault );
	// cout << "\n";
	...
	for(int i = 0; i < P; i++){
		BANDS = bandvectors_and_values(band_eigenvalues_and_eigenvectors, phase, params, neighbors,  dft_path[i][0], dft_path[i][1], dft_path[i][2], false);
		for(int j = 0; j < N; j++){
	...
	string command2= "gnuplot -e \"plot \\\"BAND_STRUCTURE_TB.dat\\\" u 1:2 w l; pause -1 \"";
